local clang = require 'bindings.luaclang-parser'
local sqlite3 = require 'bindings.lsqlite3'
...
DBG("SQLite3 version: "..sqlite3.version())
...
db:exec("CREATE TABLE modules (name PRIMARY KEY)")
db:exec("CREATE TABLE classes (module, name, parent)")
db:exec("CREATE TABLE methods (class, name, kind, access, result, signature, static, virtual, signal, slot)")
db:exec("CREATE TABLE functions (module, name, result, signature)")
db:exec("CREATE TABLE args (ismethod, parent, name, idx, type, const, defval)")

db:exec("BEGIN")

db:exec("INSERT INTO modules VALUES ('test')")
local modId = db:last_insert_rowid()

for name, class in pairs(DB) do
    db:exec("INSERT INTO classes VALUES (%d, %s, %s)" % {modId, E(name), E(class.parent)})
